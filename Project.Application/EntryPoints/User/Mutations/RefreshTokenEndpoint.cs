...
public sealed class RefreshTokenResolver(ITokenHandler tokenHandler, IGraphQL graphQL, IClaimsService claimsService, IProjectContextFactory projectContextFactory) : IRefreshTokenResolver
{
    public async Task<MutationResult<AuthenticationDto>> RefreshTokenAsync([UseFluentValidation, UseValidator<RefreshTokenInputValidator>] RefreshTokenInput input, CancellationToken ct)
    {
        ...
        var currentRefreshToken = await graphQL.ExecuteQueryAsync(
            ctx,
        ...
        var user = await graphQL.ExecuteQueryAsync(
            ctx,
